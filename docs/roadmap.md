# SkyMission Builder 고도화 계획서 (Phase 3)

## 1. 개요
현재의 GPKG 기반 대량 미션 생성 로직을 바탕으로, DJI 기체별 하드웨어 특성을 반영하고 현장 설정을 "Zero"에 가깝게 만드는 정밀 미션 생성기로 업그레이드합니다.

## 2. 주요 개발 목표

### 2.1 하드웨어 인텔리전스 (dji_enums.py 연동)
- **기체 모델 선택**: UI에서 사용자가 드론 모델(Mavic 3E, M350 등)을 선택 가능하도록 구현.
- **Payload 매칭**: 선택된 기체에 맞는 카메라/라이다 Enum 값을 WPML에 자동 주입.
- **기기별 특성 반영**: 서브 Enum 및 페이로드 위치 인덱스 자동 설정.

### 2.2 비행 디테일 설정 (Field Expansion)
- **카메라 틸트 제어**: 짐벌 피치 각도(Gimbal Pitch) 설정 기능 추가.
- **지형 추적 모드**: `surfaceRelativeHeight` 및 지형 추적 활성화 여부 제어.
- **고급 비행 옵션**: 이륙 안전 고도, 전환 속도 등을 프리셋에 완전 통합.

### 2.3 UI/UX 및 워크플로우 개선
- **프리셋 강화**: 기체 정보가 포함된 확장된 JSON 프리셋 포맷 도입.
- **에러 핸들링**: 기체 파라미터 유도 값(예: 고도 대비 속도)에 대한 유효성 검사 경고 추가.

## 3. 세부 작업 리스트

- [x] **Task 1: Backend (kml_setting_v2.py) 수정**
    - `dji_enums.py` 데이터 임포트 및 XML 주입 로직 구현. (완료)
    - 짐벌 피치 및 지형 추적 XML 노드 업데이트 기능 추가. (완료)
- [x] **Task 2: UI (ui_desktop.py) 레이아웃 개선**
    - 드론 모델 선택용 콤보박스(Combobox) 추가. (완료)
    - 짐벌 각도 및 지형 추적 설정용 위젯 배치. (완료)
    - 2단 그룹화 레이아웃 및 단위(m, m/s 등) 표기 적용. (완료)
- [x] **Task 3: 프리셋 포맷 및 문서화**
    - 기존 프리셋에 드론 모델 및 지형 팔로우 정보 포함. (완료)
    - 상세 사용자 매뉴얼(`user_manual.md`) 작성 완료. (완료)
- [ ] **Task 4: 통합 테스트 및 검증** — 코드로 확인 가능한 부분은 완료(2026-08-27), 실기 검증만 남음
    - [x] KMZ 구조 자동 검증: 루트 파일명·`ns0:` 부재·좌표 재투영·KML/WPML 값 일치·기체 enum 주입
          (`tests/test_convert.py`), 산출물 골든 대조 (`docs/dependency-diet.md`).
    - [x] waylines.wpml 의 정체 확정: 실행 웨이포인트 26개는 **원 템플릿 현장
          (33.4717N 126.3910E)의 것이 그대로** 실린다. 폴리곤은 template.kml 에만 주입된다.
          `templateType=mapping2d` — Pilot 2 가 template 에서 경로를 재생성하는 전제의 파일이다.
          inspector 가 이 간극을 재고 200m 초과 시 경고하며(`src/core/inspector.py`),
          배치 리포트에도 고정 고지를 넣었다.
    - [ ] **실기 검증 (코드로 불가능, 현장 필요)**: DJI Pilot 2 에 이 KMZ 를 임포트했을 때
          경로가 임무 폴리곤으로 재생성되는지, 기체별(M3E/M350)로 임포트가 수락되는지.
          웨이라인 직접 실행 경로(Cloud API/MSDK)를 쓸 계획이 생기면 그때는 waylines 재생성
          기능이 필수가 된다.

## 4. 기대 효과
- 현장 조종기에서 기체 정보를 다시 인식시키거나 짐벌 각도를 조절하는 시간을 완전히 제거.
- 비행 중 발생할 수 있는 데이터 일관성 문제 해결 및 미션 신뢰도 향상.
